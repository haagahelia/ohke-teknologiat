...
*⚠️ During the week it turns out that `test.mosquitto.org` had an outage and was temporarily unavailable. In case you experience issues with the mosquitto.org server, replace `test.mosquitto.org` with `broker.hivemq.com` in the connection string. Thanks Olli K. for the hint!*
