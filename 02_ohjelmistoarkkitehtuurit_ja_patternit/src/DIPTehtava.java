 ...
 * Dependency inversion-periaatetta (DIP) ja samalla 
...
/**
 * Tämä on ohjelman käyttöliittymä, joka 
 * tulostaa haluamamme asiat. Tätä luokkaa ei tarvitse muuttaa.
 */
public class DIPTehtava{
    
    private static BusinessLuokka businessLuokkaAutolle = 
        DIPConfigurator.konfiguraatioAutolle();

    private static BusinessLuokka businessLuokkaMoottoripyoralle = 
        DIPConfigurator.konfiguraatioMoottoripyorille();

    public static void main(String[] args) {
        System.out.println("Järjestelmän autolla on "+
            "ajettu keskimäärin vuodessa: "+
            businessLuokkaAutolle.laskeKeskimaaraisetKilometritPerVuosi()+
            " kilometriä");
        
        System.out.println("Järjestelmän moottoripyörälla on "+
            "ajettu keskimäärin vuodessa: "+
            businessLuokkaMoottoripyoralle.laskeKeskimaaraisetKilometritPerVuosi()+
            " kilometriä");
    }
}

/**
 * Tämän konfiguraattoriluokan tehtävä on rakentaa sovelluksen 
 * konfiguraatio. Jos sovelluksesta halutaan erilainen
 * konfiguraatio, niin tarvitsee muuttaa vain tätä 
 * luokkaa, ei itse sovelluksen luokkia. 
 * Sovelluksen määrittelyt voitaisiin myös antaa esimerkiksi
 * XML-tiedostossa, jonka tämä luokka sitten lataisi ja
 * käsittelisi.
 * Esim. Springissä on vastaava luokka.
 */
...
/**
 * Auto-luokka toteuttaa Ajoneuvo-rajapinnan asioita ja 
 * lisäksi sillä on pelkästään Auto-tyyppiin liittyviä
 * asioita 1 kpl.
 */
...
/**
 * Moottoripyora-luokka toteuttaa Ajoneuvo-rajapinnan asioita ja 
 * lisäksi sillä on pelkästään Moottoripyora-tyyppiin liittyviä
 * asioita 1 kpl.
 */
...
/**
 * Tämä luokka sisältää järjestelmän businesslogiikan, eli 
 * oikestaan sen kaikean kiinnostavan järjestelmässä, mitä 
 * tuoteomistaja on meille määritellyt.
 */
class BusinessLuokka{
    /**
     * FIXME: Tämä on väärä paikka sanoa new avainsana, nyt BusinessLuokka
     * riippuu Auto-luokasta. Tämä luokka siis alustetaan
     * tässä kiinteästi autolla. Mitä jos haluaisin saman
     * BusinessLuokan käsittelevänkin järjestelmän moottoripyörää?
     */
    Auto auto = new Auto(56000, 2018, true);

    public int laskeKeskimaaraisetKilometritPerVuosi(){
        return auto.getAjetutKilometrit() / 
            (Calendar.getInstance().get(Calendar.YEAR) - auto.getVuosimalli());
    }
